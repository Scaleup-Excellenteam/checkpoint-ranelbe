...
#define BLOCKED_IP "142.251.142.206"
...
unsigned int hook_func(void *priv, struct sk_buff *skb, const struct nf_hook_state *state)
{
    ...
    if (ip_header->saddr == in_aton(BLOCKED_IP) ||
        ip_header->daddr == in_aton(BLOCKED_IP)) {
        // Log the source and destination IP addresses
        printk(KERN_INFO "Blocked IP detected: Source IP = %pI4, Destination IP = %pI4\n",
